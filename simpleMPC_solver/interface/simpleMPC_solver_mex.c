/*
 This is an interface for simpleMPC_solver that is used by optimizerFORCES to call the solver
*/ 
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
	...
	if( !mxIsCell(param_values) ) {
		mexErrMsgTxt("simpleMPC_solver requires a cell array as input.");
	}
	...
	if( mxGetM(par) != 11 || mxGetN(par) != 1 ) {
		mexErrMsgTxt("Parameter #1 must be of size [11 x 1]");
	}
	copyMArrayToC(mxGetPr(par),params.inputvariables,11);
	...
	copyCArrayToM(output.u0_variable, mxGetPr(plhs[0]), 4);
